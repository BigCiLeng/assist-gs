...
class SplatfactoModel(Model):
    ...
    def remove_from_all_optim(self, optimizers, deleted_mask, prefix):
        param_groups = self.get_gaussian_param_groups()
        for group, param in param_groups.items():
            self.remove_from_optim(optimizers.optimizers[prefix + group], deleted_mask, param)
    ...
    def dup_in_all_optim(self, optimizers, dup_mask, n, prefix):
        param_groups = self.get_gaussian_param_groups()
        for group, param in param_groups.items():
            self.dup_in_optim(optimizers.optimizers[prefix+group], dup_mask, param, n)
    ...
    def refinement_after(self, optimizers: Optimizers, prefix, step):
        assert step == self.step
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                self.dup_in_all_optim(optimizers, split_idcs, nsamps, prefix)

                dup_idcs = torch.where(dups)[0]
                self.dup_in_all_optim(optimizers, dup_idcs, 1, prefix)
            ...
            if deleted_mask is not None:
                self.remove_from_all_optim(optimizers, deleted_mask, prefix)
            ...
            if self.step < self.config.stop_split_at and self.step % reset_interval == self.config.refine_every:
                ...
                optim = optimizers.optimizers[prefix+"opacities"]
